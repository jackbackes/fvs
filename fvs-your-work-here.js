...
const ListNode = require('./list').ListNode;
const fvsPath = './.fvs/';
const objectsPath = fvsPath + 'objects/';
const indexPath = fvsPath + 'index';
const YOUR_NAME = 'Tay Schwifty';
...
function createFVSObject(fileContents) {
    ...
    try {
        fs.statSync(objectsPath + dir + '/' + filename)
    } catch (e) {
        fs.mkdirSync(objectsPath + dir);
        fs.writeFileSync(objectsPath + dir + '/' + filename, fileContents);
    }
...
function updateIndex(index, fileName, blobRef) {
    ...
    // var indexGrab = fs.readFileSync(indexPath, 'utf8')
    // var indexContent = indexGrab === '' ? [] : indexGrab.split('\n');
    var indexContent = fs.readFileSync(indexPath, 'utf8').split('\n');
    if(indexContent[0] === '') indexContent.shift();
    var existingEntry = indexContent.indexOf(index[0]);
    if(existingEntry === -1) {
      indexContent.push( index );
      // fs.writeFileSync(indexPath, indexContent.join(''));
    } else {
      indexContent.splice(existingEntry, 1, index)
    }
    fs.writeFileSync(indexPath, indexContent.join('\n'));
...
module.exports.init = function() {
    ...
    if(fs.readdirSync('./').indexOf('.fvs') !== -1 ) throw '.fvs already exists';
    ...
    /*
      .fvs/
        ...
        HEAD
    */
    fs.mkdirSync(fvsPath);
    fs.mkdirSync(objectsPath);
    fs.mkdirSync(fvsPath+'/refs');
    fs.writeFileSync(fvsPath+'/refs/master');
    fs.writeFileSync(fvsPath+'/HEAD');
};
...
module.exports.commit = function() {
    ...
    */
    ...
    var parentHash = fs.readFileSync(fvsPath+'/refs/master', 'utf8');
    if(parentHash !== 'undefined'){
      commitString += '\nparent ' + parentHash;
    };


    var commitHash = createFVSObject(commitString);

    // step 3. point the current branch at the new commit object
    fs.writeFileSync(fvsPath+'/refs/master',commitHash);
...
module.exports.handleDefault = function() {
    throw 'FVS requires a recognized command!';
};
